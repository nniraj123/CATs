...
bs_size = 1000
rmse_bs_nonlin = np.zeros((1,bs_size))
neurons = int(5000)
nmaxpl = int(500) # max neurons per layer
nhl = round(neurons/nmaxpl) # number of hidden layers
...
class CATS(tf.keras.Model):
    ...
    def __init__(self, nvar_ref, nvar_model, J, isreg=False, lmbda=0.01, 
        ...
        if isreg:
            h1_rm = Dense(nmaxpl, activation=act_fn, use_bias=isbias, 
                          kernel_regularizer=tf.keras.regularizers.L2(l2=lmbda),
                          kernel_initializer=initializer)(inputs1)
            h2_rm = Dense(nmaxpl, activation=act_fn, use_bias=isbias,
                          kernel_regularizer=tf.keras.regularizers.L2(l2=lmbda),
                          kernel_initializer=initializer)(h1_rm)
            h3_rm = Dense(nmaxpl, activation=act_fn, use_bias=isbias,
                          kernel_regularizer=tf.keras.regularizers.L2(l2=lmbda),
                          kernel_initializer=initializer)(h2_rm)
            h4_rm = Dense(nmaxpl, activation=act_fn, use_bias=isbias,
                          kernel_regularizer=tf.keras.regularizers.L2(l2=lmbda),
                          kernel_initializer=initializer)(h3_rm)
            h5_rm = Dense(nmaxpl, activation=act_fn, use_bias=isbias,
                          kernel_regularizer=tf.keras.regularizers.L2(l2=lmbda),
                          kernel_initializer=initializer)(h4_rm)
            out_rm = Dense(nvar_model, activation=act_fn, use_bias=isbias,
                           kernel_regularizer=tf.keras.regularizers.L2(l2=lmbda),
                           kernel_initializer=initializer)(h5_rm)
            ...
            h5_rm = Dense(nmaxpl, activation=act_fn, use_bias=isbias, 
                          kernel_initializer=initializer)(h4_rm)
            h6_rm = Dense(nmaxpl, activation=act_fn, use_bias=isbias, 
                          kernel_initializer=initializer)(h5_rm)
            h7_rm = Dense(nmaxpl, activation=act_fn, use_bias=isbias, 
                          kernel_initializer=initializer)(h6_rm)
            h8_rm = Dense(nmaxpl, activation=act_fn, use_bias=isbias, 
                          kernel_initializer=initializer)(h7_rm)
            h9_rm = Dense(nmaxpl, activation=act_fn, use_bias=isbias, 
                          kernel_initializer=initializer)(h8_rm)
            h10_rm = Dense(nmaxpl, activation=act_fn, use_bias=isbias, 
                          kernel_initializer=initializer)(h9_rm)
            out_rm = Dense(nvar_model, activation=act_fn, use_bias=isbias, 
                           kernel_initializer=initializer)(h10_rm)
        ...
        if isreg:
            h1_mr = Dense(nmaxpl, activation=act_fn, use_bias=isbias, 
                          kernel_initializer=initializer,
                          kernel_regularizer=tf.keras.regularizers.L2
                          (l2=lmbda))(inputs3)
            h2_mr = Dense(nmaxpl, activation=act_fn, use_bias=isbias, 
                          kernel_initializer=initializer,
                          kernel_regularizer=tf.keras.regularizers.L2
            (l2=lmbda))(h1_mr)
            h3_mr = Dense(nmaxpl, activation=act_fn, use_bias=isbias, 
                          kernel_initializer=initializer,
                          kernel_regularizer=tf.keras.regularizers.L2
            (l2=lmbda))(h2_mr)
            h4_mr = Dense(nmaxpl, activation=act_fn, use_bias=isbias, 
                          kernel_initializer=initializer,
                          kernel_regularizer=tf.keras.regularizers.L2
            (l2=lmbda))(h3_mr)
            h5_mr = Dense(nmaxpl, activation=act_fn, use_bias=isbias, 
                          kernel_initializer=initializer,
                          kernel_regularizer=tf.keras.regularizers.L2
            (l2=lmbda))(h4_mr)
            ...
            h5_mr = Dense(nmaxpl, activation=act_fn, use_bias=isbias, 
                          kernel_initializer=initializer)(h4_mr)
            h6_mr = Dense(nmaxpl, activation=act_fn, use_bias=isbias, 
                          kernel_initializer=initializer)(h5_mr)
            h7_mr = Dense(nmaxpl, activation=act_fn, use_bias=isbias, 
                          kernel_initializer=initializer)(h6_mr)
            h8_mr = Dense(nmaxpl, activation=act_fn, use_bias=isbias, 
                          kernel_initializer=initializer)(h7_mr)
            h9_mr = Dense(nmaxpl, activation=act_fn, use_bias=isbias, 
                          kernel_initializer=initializer)(h8_mr)
            h10_mr = Dense(nmaxpl, activation=act_fn, use_bias=isbias, 
                          kernel_initializer=initializer)(h9_mr)
            out_mr = Dense(nvar_ref, activation=act_fn, use_bias=isbias, 
                           kernel_initializer=initializer)(h10_mr)
...
rmse_bs_nonlin[0,:] = bootstrap(residual,size=bs_size)
del states_ANN
...
fname = f"CATs_L96_RMSE_vs_Neurons_Nonlin-"+ \
f"{nhl}HL_{int(neurons)}Neurons_tau={n_mtu}_LRcorrected.nc"
ncfile = nc.Dataset(fname,mode='w',format='NETCDF4')

# Add dimensions
nneurons = ncfile.createDimension('nneurons',len(neurons_arr))
