...
bs_size = 1000
rmse_bs_MLonly = np.zeros((len(n_neurons),bs_size))
...
for nn in range(len(n_neurons)):
    ...
    rmse_bs_MLonly[nn,:] = bootstrap(residual,size=bs_size)
    del states_ANN
    gc.collect()
...
fname = f"CATs_L96_RMSE_vs_Neurons_MLonly_2HL_"+\
f"{n_neurons[0]}-{n_neurons[-1]}Neurons_tau={n_mtu}_LRcorrected.nc"
