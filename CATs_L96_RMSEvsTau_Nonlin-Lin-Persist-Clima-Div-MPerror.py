...
bs_size = 1000
rmse_bs_nonlin = np.zeros((len(n_mtu_arr),bs_size))
rmse_bs_persist_perfect = np.zeros((len(n_mtu_arr),bs_size))
rmse_bs_linearCATs = np.zeros((len(n_mtu_arr),bs_size))
rmse_bs_diverge = np.zeros((len(n_mtu_arr),bs_size))
rmse_bs_clima = np.zeros((len(n_mtu_arr),bs_size))
rmse_bs_mperror = np.zeros((len(n_mtu_arr),bs_size))
...
for tau_n in range(len(n_mtu_arr)):
    ...
    #+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    #+-+-+-+-+-+-+-+-+ Bootstrapped RMSE -- Nonlinear CATs +-+-+-+-+-+-+-+-+-+-+
    #+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    rmse_bs_nonlin[tau_n,:] = bootstrap(residual,size=bs_size)
    del states_ANN

    #+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    #+-+-+-+-+-+-+-+-+-+-+- Bootstrapped RMSE -- Linear CATs +-+-+-+-+-+-+-+-+-+
    #+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    # CATS definition parameters
    def_params = {'isreg':False, 'lmbda':lmbda**2, 
                  'act_fn':'tanh', 'isbias':True,
    ...
    for kk in range(n_test):
        states_zeroweight[kk,:] = cat_l96_zeroweight.forecast(np.reshape(
            x_train[n_train+kk,:], [1,-1]), batch_size=1)
    
    # Scale the zero weight outputs
    states_zeroweight = scaler.inverse_transform(states_zeroweight)

    # Compute RMSE
    residual_zeroweight = states_twolevel[n_train+nts_fcast:
                                          n_train+n_test+nts_fcast,:] - \
    states_zeroweight
    
    # Bootstrap
    rmse_bs_linearCATs[tau_n,:] = bootstrap(residual_zeroweight,size=bs_size)
    
    del states_zeroweight
    #+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    #+-+-+-+-+-+- Bootstrapped RMSE -- Persistence Perfect Model +-+-+-+-+-+-+-+
    #+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    residual_persist_perfect = states_twolevel[n_train+nts_fcast:n_train + \
                                               nts_fcast+n_test,:K] - \
    states_twolevel[n_train:n_train+n_test,:K]
    
    rmse_bs_persist_perfect[tau_n,:] = bootstrap(residual_persist_perfect,
                                                 size=bs_size)
    
    #+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    #+-+-+-+-+-+-+- Bootstrapped RMSE -- Machine Precision Error +-+-+-+-+-+-+-+
    #+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    np.random.seed(100)
    mp_error = np.random.normal(0,1e-14) # machine precision error
    states_mperror = np.zeros((n_test,K+J*K))
    
    for kk in range(n_test):
        ic_ = states_twolevel[n_train+kk] + mp_error
        fc_ = odeint(L96_twolevel,ic_,t_fcast,args=args_twolevel)[-1,:]
        states_mperror[kk,:] = fc_    
    
    residual_mperror = states_twolevel[n_train+nts_fcast:n_train + \
                                       nts_fcast+n_test,:K] - \
    states_mperror[:,:K]
    
    # Bootstrap
    rmse_bs_mperror[tau_n,:] = bootstrap(residual_mperror,size=bs_size)
    
    del states_mperror
    gc.collect()
    
#+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
#+-+-+-+-+-+-+-+-+-+- Bootstrapped RMSE -- Climatology +-+-+-+-+-+-+-+-+-+-+-+-+
#+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
residual_clima = states_twolevel[n_train:,:K]-np.mean(states_twolevel[:,:K],\
                                                      axis=0)
rmse_bs_clima = bootstrap(residual_clima,size=bs_size)

#+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
#+-+-+-+-+-+-+-+-+-+-+- Bootstrapped RMSE -- Divergence +-+-+-+-+-+-+-+-+-+-+-+-
#+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
rmse_bs_diverge = np.sqrt(2)*rmse_bs_clima
...
fname = f"CATs_L96_RMSE_vs_Tau_Nonlin-Lin-Persist-Clima-"+ \
f"Div-MPerror_1HL_10Neurons_tau={n_mtu_arr[0]}-{n_mtu_arr[-1]}_LRcorrected.nc"
...
rmse_nonlincats = ncfile.createVariable('rmse_nonlincats',\
                                        np.float32, ('ntau','nbs'))
rmse_nonlincats.long_name = 'RMSE for Nonlinear CATs'

rmse_lincats = ncfile.createVariable('rmse_lincats',\
                                     np.float32, ('ntau','nbs'))
rmse_lincats.long_name = 'RMSE for Linear CATs'

rmse_persist = ncfile.createVariable('rmse_persist',\
                                     np.float32, ('ntau','nbs'))
rmse_persist.long_name = 'RMSE for Persistence -- Perfect Model'

rmse_clima = ncfile.createVariable('rmse_clima',\
                                     np.float32, ('ntau','nbs'))
rmse_clima.long_name = 'RMSE for Climatology Forecast'

rmse_diverge = ncfile.createVariable('rmse_diverge',\
                                     np.float32, ('ntau','nbs'))
rmse_diverge.long_name = 'RMSE for Divergence'

rmse_mperror = ncfile.createVariable('rmse_mperror',\
                                     np.float32, ('ntau','nbs'))
rmse_mperror.long_name = 'RMSE for Machine Precision Error'

# write the data
tau[:] = n_mtu_arr
rmse_nonlincats[:,:] = rmse_bs_nonlin
rmse_lincats[:,:] = rmse_bs_linearCATs
rmse_persist[:,:] = rmse_bs_persist_perfect
rmse_clima[:,:] = rmse_bs_clima
rmse_diverge[:,:] = rmse_bs_diverge
rmse_mperror[:,:] = rmse_bs_mperror
