...
class CATS(tf.keras.Model):
    ...
    def __init__(self, nvar_ref, nvar_model, J, isreg=False, lmbda=0.01, 
        ...
        if isreg:
            ...
            h2_mr = Dense(10, activation=act_fn, use_bias=isbias, 
                          kernel_initializer=initializer,
...
bs_size = 1000
rmse_bs_tmr_alone = np.zeros((len(n_mtu_arr),bs_size))
...
for tau_n in range(len(n_mtu_arr)):
    ...
    rmse_bs_tmr_alone[tau_n,:] = bootstrap(residual,size=bs_size)
    del states_ANN

#+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
#+-+-+-+-+-+-+-+-+-+-+-+-+- Save bootstrapped RMSE -+-+-+-+-+-+-+-+-+-+-+-+-+-+-
#+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
try:ncfile.close()
except:pass  

fname = f"CATs_L96_RMSE_vs_Tau_Tmr-alone_"+ \
f"2HL_10Neurons_tau={n_mtu_arr[0]}-{n_mtu_arr[-1]}_LRcorrected.nc"
