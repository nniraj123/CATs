...
rmse_bs_nonlin = np.zeros((len(n_neurons),bs_size))
rmse_bs_linearCATs = np.zeros((len(n_neurons),bs_size))
...
for nn in range(len(n_neurons)):
    ...
    rmse_bs_nonlin[nn,:] = bootstrap(residual,size=bs_size)
    del states_ANN
...
f"1HL_{n_neurons[0]}-{n_neurons[-1]}Neurons_tau={n_mtu}_LRcorrected.nc"
